...
app.get("/login", (req, res) => {
  const scope =
    "user-read-private user-read-email streaming playlist-read-private user-read-playback-state";
...
app.get("/callback", async (req, res) => {
  ...
  try {
    ...
    res.redirect(
      `http://localhost:6704/callback?access_token=${access_token}&refresh_token=${refresh_token}&expires_in=${expires_in}`
    );
  } catch (error) {
...
app.get("/refresh", async (req, res) => {
  try {
    ...
    if (!tokenDoc || !tokenDoc.refreshToken) {
      return res
        .status(404)
        .json({ error: "Không tìm thấy refresh token trong MongoDB" });
    }
